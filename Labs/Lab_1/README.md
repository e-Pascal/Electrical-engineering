...
2.  Измерить напряжение холостого хода ($U_{хх}$) и ток ($I_{хх}$) на выходе активного двухполюсника. Рассчитать мощность холостого хода ($P_{хх}$). Результат зафиксировать в таблице в рабочей тетради, форма приведена в [шаблоне](./templates/template_1.docx);
...
4.  Рассчитать внутреннее сопротивление активного двухполюсника по формуле. Результат зафиксировать в таблице в рабочей тетради, форма приведена в [шаблоне](./templates/template_1.docx);

```math
R_0=\frac{U_{xx}}{I_{кз}}
```

5.  Подключить резистор $R$ (по варианту из `таблицы 1`) к выходу активного двухполюсника и измерить ток ($I$) и напряжение ($U$) на нём.  Рассчитать мощность ($P$). Результат зафиксировать в таблице в рабочей тетради, форма приведена в [шаблоне](./templates/template_1.docx);
6.  Активный двухполюсник заменить эквивалентной ЭДС, с последовательно включенным эквивалентным сопротивлением (`теорема Тевенина – Гельмгольца`), а затем последовательно подключить сопротивление $R$ (по варианту из `таблицы 1`) и измерить ток ($I$) и напряжение ($U$) на нём.  Рассчитать мощность ($P$). Результат зафиксировать в таблице в рабочей тетради, форма приведена в [шаблоне](./templates/template_1.docx);
...
2.  Получить дополнительные значения токов по формулам, изменяя сопротивление нагрузки ($R_н$). Рассчитать мощность $P_1$ и $P_2$. Собрать электрические схемы. Результат зафиксировать в таблице в рабочей тетради, форма приведена в [шаблоне](./templates/template_1.docx);
...
4.  Построить графики друг под другом:
    * вольт-амперной характеристики (ВАХ);
    * зависимости мощности ($P$) от тока ($I$);
    * зависимости мощности ($P$) от нагрузочного резистора ($R_н$). 

### 3. Исследование поведения резистора, конденсатора и катушки индуктивности в цепи постоянного тока
...
6.   Вольт-амперная характерисика (ВАХ)
...
## Примеры решения типовых задач для допуска

### Задача 1.
1.1 Рассчитать значения токов I1 и I2, протекающих через резисторы R1 и R2 соответственно.
1.2 Рассчитать значения напряжений U1 и U2 на резисторах R1 и R2 соответственно.
![./pic/p_6.png](./pic/p_6.png)
#### Решение
1.1 Резисторы R1 и R2 подключены к источнику ЭДС последовательно, поэтому через данные резисторы будет протекать одинаковый ток:
$I_1=I_2=I\frac{E}{(R_1+R_2)}=\frac{5 В}{(40 Ом + 60 Ом)} = 0,05 А = 50 мА$
