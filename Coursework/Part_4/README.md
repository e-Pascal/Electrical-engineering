...
Каждый пользовательский вывод МК может использоваться в режиме:
...
##### 4.5. Принцип работы
UART передает данные по асинхронному принципу, без использования тактового сигнала.
...
##### 5.5. Принцип работы
SPI использует несколько линий для синхронного обмена данными.
...
##### 6.5. Принцип работы
I2C использует двухпроводную шину для обмена данными, где одно устройство является ведущим, а другие — подчиненными.
...
##### 7.5. Принцип работы
CAN использует дифференциальный сигнал для передачи данных с высокой помехоустойчивостью.
...
##### 8.5. Принцип работы
USB использует два режима связи: **модульный** и **пакетный**. В модульном режиме данные передаются с использованием сигналов "пакет/ответ". В более поздних версиях USB, таких как USB 3.0 и выше, используется многоканальная передача данных для увеличения скорости.
...
##### 9.5. Принцип работы
PCI использует параллельную передачу данных, с разделением на адресные и управляющие линии. Это позволяет нескольким устройствам подключаться к одной шине и обмениваться данными с высокой пропускной способностью.
...
##### 10.5. Принцип работы
PCIe использует несколько линий передачи для последовательной передачи данных. Каждая линия может работать в двух направлениях (full-duplex), что повышает пропускную способность. PCIe передает данные по «каналам», которые могут использоваться для разных устройств в рамках одного слота.
