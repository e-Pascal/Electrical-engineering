...
Рассчитаем необходимый номинал катушки индуктивности. Используя формулу из спецификации, рассчитаем константу индуктивности
```math
V \cdot T (В \cdot мксек) = (U_{in} - U_{out}) \cdot \frac{U_{out}}{U_{in}} \cdot \frac{1000}{f (кГн)}
```
```math
V \cdot T (В \cdot мксек) = (26 - 7) \cdot \frac{7}{26} \cdot \frac{1000}{52 (кГн)} ≈ 98
```
$R_{н}\ = 10 \ Ом$. По закону Ома выходной ток $I_{out}$ равен
